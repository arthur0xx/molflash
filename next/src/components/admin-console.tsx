...
import { Ban, Boxes, CreditCard, FolderTree, LayoutDashboard, ListChecks, Menu, MessageCircle, Package2, Pencil, Plus, Send, Settings2, ShieldAlert, ShieldCheck, Trash2, UserRound, UserX, UsersRound, X } from "lucide-react";
...
const statusTransitions: Record<OrderStatus, OrderStatus[]> = {
  new: ["processing", "waiting", "rejected"],
...
export function AdminConsole() {
  ...
  const filteredCustomerOrders = selectedCustomerId ? allOrders.filter((order) => order.customerId === selectedCustomerId) : allOrders;
  ...
  const overviewOrders = [...allOrders].filter((order) => order.status === "new" || order.status === "waiting" || order.status === "processing").sort((left, right) => {
...
function OrderGrid({ orders, data, onStatus, deliveryDrafts, deliveryNotes, onDeliveryDraft, onDeliveryNote, onSendDelivery, onLoadPurchaseUrl }: { orders: DisplayOrder[]; data: StoreSnapshot; onStatus: (id: string, status: OrderStatus) => void; deliveryDrafts: Record<string, string>; deliveryNotes: Record<string, string>; onDeliveryDraft: React.Dispatch<React.SetStateAction<Record<string, string>>>; onDeliveryNote: React.Dispatch<React.SetStateAction<Record<string, string>>>; onSendDelivery: (id: string) => void; onLoadPurchaseUrl: (orderId: string) => Promise<string | null> }) {
