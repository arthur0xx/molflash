...
import { Archive, ArchiveRestore, Ban, Banknote, BookOpen, Boxes, CheckCircle2, CircleOff, Clock3, CreditCard, FolderTree, Landmark, LayoutDashboard, ListChecks, Menu, MessageCircle, Package2, Pencil, Plus, Send, Settings2, ShieldAlert, ShieldCheck, Trash2, UserRound, UserX, UsersRound, X } from "lucide-react";
...
import { getCatalogReadiness, getOwnerWorkItems } from "@/lib/operations";
...
type Tab = "overview" | "orders" | "products" | "categories" | "blog" | "customers" | "support" | "team" | "payments" | "settings";
...
function CatalogReadinessBadge({ service, categories }: { service: Service; categories: Category[] }) {
  const readiness = getCatalogReadiness(service, categories);
  return <span className={`catalog-readiness ${readiness.state}`} title={readiness.missing.length ? `ينقص: ${readiness.missing.join("، ")}` : readiness.label}>{readiness.label}{readiness.missing.length ? ` · ${readiness.missing.length}` : ""}</span>;
}
...
export function AdminConsole() {
  ...
  const ownerWorkItems = useMemo(() => isManager ? [] : getOwnerWorkItems(data, tickets), [data, isManager, tickets]);
  ...
  const navItems: [Tab, string][] = isManager
    ...
    : [["overview", "اليوم"], ["orders", "الطلبات"], ["products", "المنتجات"], ["categories", "التصنيفات"], ["blog", "المدونة"], ["customers", "العملاء"], ["support", "الدعم"], ["team", "الفريق"], ["payments", "إدارة الدفع"], ["settings", "الإعدادات"]];
  ...
  const tabTitles: Record<Tab, string> = { overview: "نظرة عامة CMC", orders: "إدارة الطلبات", products: "مجلدات المنتجات", categories: "إدارة التصنيفات", customers: "العملاء والمحافظ", support: "رسائل الدعم", team: "إدارة الفريق", payments: "مركز الدفع", blog: "إدارة المدونة", settings: "إعدادات المتجر" };
  ...
  return <div className="admin-console"><aside className="cmc-sidebar" aria-label="تنقّل CMC"><div className="cmc-title">ChriGsm <b>CMC</b></div><nav>{navItems.map(([id, label]) => <button className={tab === id ? "active" : ""} onClick={() => setTab(id)} aria-current={tab === id ? "page" : undefined} key={id}>{label}</button>)}</nav><div className="cmc-summary"><b>لوحة الإدارة</b><span>أدر الطلبات والخدمات والعملاء من مكان واحد</span></div></aside><nav className="cmc-mobile-nav" aria-label="تنقّل CMC على الهاتف">{mobilePrimaryNav.map(([id, label]) => <button className={tab === id ? "active" : ""} type="button" onClick={() => { setTab(id); setMobileMoreOpen(false); }} aria-current={tab === id ? "page" : undefined} key={id}>{id === "overview" ? <LayoutDashboard size={18}/> : id === "orders" ? <ListChecks size={18}/> : id === "products" ? <Package2 size={18}/> : <UsersRound size={18}/>}<span>{label}</span></button>)}{mobileMoreNav.length > 0 && <button type="button" className={mobileMoreNav.some(([id]) => tab === id) ? "active" : ""} onClick={() => setMobileMoreOpen(true)} aria-expanded={mobileMoreOpen} aria-controls="cmc-mobile-more"><Menu size={18}/><span>المزيد</span></button>}</nav>{mobileMoreOpen && <div className="cmc-more-backdrop" role="presentation" onMouseDown={() => setMobileMoreOpen(false)}><section id="cmc-mobile-more" className="cmc-more-sheet" role="dialog" aria-modal="true" aria-label="وجهات CMC الإضافية" onMouseDown={(event) => event.stopPropagation()}><div className="cmc-sheet-handle"/><header><div><p className="eyebrow">CMC</p><h2>المزيد من الأدوات</h2></div><button className="icon-action-button" type="button" aria-label="إغلاق" onClick={() => setMobileMoreOpen(false)}><X size={19}/></button></header><div className="cmc-more-list">{mobileMoreNav.map(([id, label]) => <button type="button" className={tab === id ? "active" : ""} onClick={() => { setTab(id); setMobileMoreOpen(false); }} key={id}>{id === "categories" ? <FolderTree size={19}/> : id === "blog" ? <BookOpen size={19}/> : id === "support" ? <MessageCircle size={19}/> : id === "team" ? <UsersRound size={19}/> : <Settings2 size={19}/>}<span>{label}</span></button>)}</div></section></div>}<section className="cmc-content"><header className="cmc-heading"><div><p className="eyebrow">إدارة العمليات</p><h1>{tab === "overview" && !isManager ? "اليوم في المتجر" : tabTitles[tab]}</h1></div><span className="live-pill"><span/> إدارة المتجر</span></header><p className="admin-notice" role="status">{notice}</p><AdminOnboardingTour role={isManager ? "manager" : "admin"}/>
    {tab === "overview" && <><div className="metric-grid"><Metric icon={<UsersRound/>} label="عملاء نشطون" value={String(data.customers.length)} note="حسابات مسجلة"/><Metric icon={<Boxes/>} label="طلبات جديدة" value={String(allOrders.filter((item) => item.status === "new").length)} note="تصل من المتجر"/><Metric icon={<FolderTree/>} label="قيد المعالجة" value={String(processing)} note="بيانات مقفلة للعميل"/><Metric icon={<CreditCard/>} label="إجمالي الأرصدة" value={formatMAD(totalWallet)} note="محافظ العملاء"/></div>{!isManager && <section className="cmc-card owner-workboard"><div className="section-title"><div><p className="eyebrow">قرارك التالي</p><h2>ما يحتاج تدخلك اليوم</h2></div><span className="muted-text">{ownerWorkItems.length} عناصر</span></div><p className="muted-text">هذه قائمة تنظيمية فقط؛ لا تؤكد الدفع ولا تنشر خدمة ولا تغيّر أي بيانات حتى تفتح الإجراء المناسب بنفسك.</p>{ownerWorkItems.length ? <div className="owner-work-list">{ownerWorkItems.map((item) => <article className={`owner-work-item ${item.tone}`} key={item.id}><span>{item.kind === "order" ? <Boxes size={18}/> : item.kind === "support" ? <MessageCircle size={18}/> : item.kind === "payment" ? <Landmark size={18}/> : <Package2 size={18}/>}</span><div><b>{item.title}</b><p>{item.detail}</p><small>{item.createdAt ? `منذ ${item.createdAt.slice(0, 10)}` : "يحتاج مراجعة"}</small></div><button className="filter-button" type="button" onClick={() => setTab(item.targetTab)}>فتح</button></article>)}</div> : <div className="empty-state"><CheckCircle2 size={24}/><h2>لا توجد عناصر معلقة الآن</h2><p>عند وصول طلب أو رسالة دعم أو تحويل أو نقص في خدمة، سيظهر هنا دون تنفيذ أي قرار تلقائي.</p></div>}</section>}<section className="cmc-card"><div className="section-title"><div><p className="eyebrow">تحتاج إجراءً الآن</p><h2>طابور الطلبات</h2></div><button className="filter-button" type="button" onClick={() => { setOrderStatusFilter("all"); setOrderSearch(""); setSelectedCustomerId(null); setTab("orders"); }}>فتح كل الطلبات</button></div><p className="muted-text">تظهر الطلبات الجديدة أو التي تحتاج متابعة فقط. تُنفذ التفاصيل والتسليم من قسم الطلبات.</p>{overviewOrders.length ? <OrderGrid orders={overviewOrders} data={data} onStatus={updateOrder} deliveryDrafts={deliveryDrafts} deliveryNotes={deliveryNotes} onDeliveryDraft={setDeliveryDrafts} onDeliveryNote={setDeliveryNotes} onSendDelivery={sendDelivery} onLoadPurchaseUrl={loadOrderPurchaseUrl}/> : <div className="empty-state"><Boxes size={24}/><h2>لا توجد طلبات تحتاج إجراءً الآن</h2><p>ستظهر هنا الطلبات الجديدة أو التي تنتظر متابعة منك.</p></div>}</section></>}
    ...
    {tab === "blog" && !isManager && <AdminBlogManager onNotice={setNotice}/>}
