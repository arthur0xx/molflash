...
import { Boxes, CreditCard, FolderTree, MessageCircle, Pencil, Plus, Send, Settings2, Trash2, UserRound, UsersRound } from "lucide-react";
import type { Category, StoreSnapshot, Order, OrderStatus, Service, SupportTicket } from "@/lib/types";
...
export function AdminConsole({ initial }: { initial: StoreSnapshot }) {
  ...
  return <div className="admin-console"><aside className="cmc-sidebar"><div className="cmc-title">ChriGsm <b>CMC</b></div><nav>{([ ["overview", "نظرة عامة"], ["orders", "الطلبات"], ["products", "المنتجات"], ["categories", "التصنيفات"], ["customers", "العملاء"], ["support", "الدعم"], ["settings", "الإعدادات"] ] as [Tab, string][]).map(([id, label]) => <button className={tab === id ? "active" : ""} onClick={() => setTab(id)} key={id}>{label}</button>)}</nav><div className="cmc-summary"><b>لوحة الإدارة</b><span>أدر الطلبات والخدمات والعملاء من مكان واحد</span></div></aside><section className="cmc-content"><header className="cmc-heading"><div><p className="eyebrow">إدارة العمليات</p><h1>{({ overview: "نظرة عامة CMC", orders: "إدارة الطلبات", products: "مجلدات المنتجات", categories: "إدارة التصنيفات", customers: "العملاء والمحافظ", support: "رسائل الدعم", settings: "إعدادات المتجر" } as Record<Tab, string>)[tab]}</h1></div><span className="live-pill"><span/> إدارة المتجر</span></header><p className="admin-notice">{notice}</p>
    ...
    {tab === "customers" && <section className="cmc-card"><div className="section-title"><div><p className="eyebrow">إدارة الأرصدة</p><h2>العملاء</h2></div><span className="muted-text">أدخل المبلغ والسبب قبل التنفيذ</span></div><div className="customer-grid">{data.customers.map((customer) => <article className="customer-card large" key={customer.id}><span className="avatar">{customer.avatarUrl ? <img src={customer.avatarUrl} alt={`صورة ${customer.fullName}`}/> : <UserRound size={19} aria-label="صورة افتراضية"/>}</span><div><h3>{customer.fullName}</h3><p>{customer.phone}</p><strong>رصيد {formatMAD(walletBalance(customer.walletMad))} · {allOrders.filter((order) => order.customerId === customer.id).length} طلبات</strong></div><div className="wallet-actions"><input type="number" inputMode="decimal" min="0.01" max="1000000" step="0.01" aria-label={`مبلغ تعديل رصيد ${customer.fullName}`} value={walletAmounts[customer.id] || ""} onChange={(event) => setWalletAmounts((previous) => ({ ...previous, [customer.id]: event.target.value }))} placeholder="المبلغ (د.م.)" disabled={!firebase || walletSavingId === customer.id}/><input aria-label={`سبب تعديل رصيد ${customer.fullName}`} value={walletReasons[customer.id] || ""} onChange={(event) => setWalletReasons((previous) => ({ ...previous, [customer.id]: event.target.value }))} placeholder="سبب التعديل (مطلوب)" disabled={!firebase || walletSavingId === customer.id}/><div><button className="wallet-credit" onClick={() => submitWalletAdjustment(customer.id, "credit")} disabled={!firebase || walletSavingId === customer.id}>إضافة رصيد</button><button className="wallet-debit" onClick={() => submitWalletAdjustment(customer.id, "debit")} disabled={!firebase || walletSavingId === customer.id}>خصم رصيد</button><button onClick={() => { setSelectedCustomerId(customer.id); setTab("orders"); }} disabled={walletSavingId === customer.id}>طلبات</button></div></div></article>)}</div></section>}
