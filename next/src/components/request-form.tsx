...
export function RequestForm({ service }: { service: Service }) {
  ...
  if (orderId) {
    if (createdPayment) return <div className="request-payment-success" role="status"><CheckCircle2 size={22}/><div><b>{createdPayment.payment.status === "proof_submitted" ? "وصل إثبات التحويل للمراجعة." : "أصبح طلبك بانتظار التحقق من التحويل."}</b><p>حوّل المبلغ وفق التعليمات ثم اكتب المرجع التالي كما هو في سبب التحويل:</p><button type="button" className="payment-reference-copy" onClick={() => { void copyReference(createdPayment.payment.paymentReference); }} aria-label="نسخ مرجع التحويل"><code>{createdPayment.payment.paymentReference}</code><Copy size={15}/></button><PaymentTransferDetails methodSnapshot={createdPayment.payment.methodSnapshot} paymentReference={createdPayment.payment.paymentReference} fallbackInstructions={createdPayment.instructions}/>{createdPayment.payment.status === "manual_transfer_pending" && <label className="payment-proof-upload"><ImageUp size={16}/><span>{proofUploading ? "جارٍ رفع الإثبات..." : "إرفاق سكرين التحويل"}</span><input type="file" accept="image/png,image/jpeg,image/webp" disabled={proofUploading} onChange={(event) => { const file = event.currentTarget.files?.[0]; if (file) void uploadProof(file); event.currentTarget.value = ""; }}/></label>}{createdPayment.payment.status === "proof_submitted" && <small className="payment-proof-confirmed"><CheckCircle2 size={14}/> سيظهر الإثبات للمالك لمطابقته مع التحويل البنكي.</small>}<small><ShieldCheck size={14}/> لا يؤكد المرجع أو الصورة الدفع تلقائيًا؛ سيبدأ تنفيذ الطلب بعد المراجعة اليدوية.</small><Link href="/account">متابعة الطلب من حسابي <span aria-hidden="true">←</span></Link>{error && <p className="form-error" role="alert">{error}</p>}</div></div>;
