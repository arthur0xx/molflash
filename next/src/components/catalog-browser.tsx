...
export function CatalogBrowser({ services, categories, initialCategory }: CatalogBrowserProps) {
  ...
  return <>
    ...
    <div className="category-filter-row" aria-label="تصفية حسب التصنيف">
      ...
      {categories.map((item) => <button key={item.id} type="button" className={category === item.id ? "filter-active" : ""} onClick={() => setCategory(item.id)}>{item.name} <span>{services.filter((service) => service.categoryId === item.id).length}</span></button>)}
    </div>
