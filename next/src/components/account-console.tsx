...
import { PaymentTransferDetails } from "@/components/payment-transfer-details";
import { requestSignedMediaUpload, uploadSignedMediaImage } from "@/lib/media-upload";
import { getOrderNextStep } from "@/lib/operations";
...
export function AccountConsole() {
  ...
  useEffect(() => {
    if (!notice) return;
    const timeout = window.setTimeout(() => setNotice((current) => current?.id === notice.id ? null : current), 4200);
    return () => window.clearTimeout(timeout);
  }, [notice]);

  useEffect(() => {
    let active = true;
    void completeGoogleLinkRedirect().then((result) => {
      if (!active || !result) return;
      if (result === "linked" || result === "already-linked") { setGoogleLinked(true); setGoogleLinkState("linked"); return; }
      setGoogleLinkState(result === "conflict" ? "conflict" : "error");
    });
    return () => { active = false; };
  }, []);
  ...
  async function submitProfile(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setProfileSaved(true); showNotice("تم حفظ تحديثات حسابك.");
  ...
  async function submitSupport(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setTickets((previous) => [result.ticket!, ...previous]); setTicketSaved(true); formElement.reset(); showNotice("تم إرسال طلب الدعم إلى الفريق.");
  ...
  async function confirmPhoneVerification() {
    ...
    try {
      ...
      setPhoneVerificationCode(""); setPhoneVerificationState("verified"); showNotice("تم تأكيد رقم واتساب بنجاح.");
  ...
  async function saveNotificationPreferences(preferences: CustomerProfile["notificationPreferences"]) {
    ...
    try {
      ...
      setCustomer((previous) => previous ? { ...previous, ...result.profile, notificationPreferences: preferences } : previous); showNotice("تم حفظ تفضيلات الإشعارات.");
  ...
  async function connectGoogleAccount() {
    ...
    if (result === "linked" || result === "already-linked") { setGoogleLinked(true); setGoogleLinkState("linked"); return; }
    if (result === "redirecting") { setGoogleLinkState("redirecting"); return; }
    setGoogleLinkState(result === "conflict" ? "conflict" : "error");
  }
  ...
  if (accountState === "loading") return <main className="store-shell account-shell" aria-busy="true"><section className="account-loading" aria-label="جارٍ تحميل الحساب"><div className="account-loading-copy"><p className="eyebrow">منطقة العميل</p><h1>جارٍ فتح حسابك…</h1><p className="panel-intro">نحضّر معلوماتك وطلباتك بأمان.</p></div><div className="account-loading-metrics"><span/><span/><span/><span/></div><div className="account-loading-panel"/></section></main>;
...
function AccountNotice({ notice, onDismiss }: { notice: AccountNotice; onDismiss: () => void }) {
  ...
  return <div className={`account-notice ${notice.tone}`} role="status" aria-live="polite"><CheckCircle2 size={18}/><span>{notice.message}</span><button type="button" onClick={onDismiss} aria-label="إغلاق الإشعار">×</button></div>;
}
