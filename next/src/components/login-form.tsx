...
export function LoginForm() {
  ...
  function setActiveMode(nextMode: LoginMode) { setMode(nextMode); setError(""); setResetMessage(""); }
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(false);
    if (!session) { setError("تعذر تسجيل الدخول. تحقق من البريد الإلكتروني وكلمة المرور."); return; }
    if (session.role !== "admin" && !session.emailVerified) { router.push(verificationPath); return; }
    router.push(session.role === "admin" ? "/admin" : safeNext);
  }

  async function submitGoogle() {
    setSubmittingGoogle(true); setError("");
    const result = await signInWithGoogle();
    setSubmittingGoogle(false);
    if (result === "existing-account") { setError("هذا البريد مرتبط بطريقة دخول أخرى. سجّل الدخول بالبريد وكلمة المرور أولًا."); return; }
    if (result !== "signed-in") { setError("تعذر تسجيل الدخول عبر Google حاليًا. حاول مرة أخرى."); return; }
    const session = await signInWithGoogleSession();
    if (!session) { setError("تعذر فتح جلسة الحساب بعد تسجيل الدخول عبر Google."); return; }
    router.push(session.role === "admin" ? "/admin" : safeNext);
  }

  async function signInWithGoogleSession() {
  ...
  async function submitSignup(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmittingSignup(true); setError("");
    const result = await registerCustomer(fullName, phone, email, password);
    setSubmittingSignup(false);
    if (result === "created") { router.push(`${verificationPath}&sent=1`); return; }
    if (result === "verification-unavailable") { router.push(`${verificationPath}&send=retry`); return; }
    if (result === "email-in-use") { setError("هذا البريد مرتبط بحساب موجود. سجّل الدخول أو استعد كلمة المرور."); return; }
    if (result === "weak-password") { setError("استخدم اسمًا صحيحًا وكلمة مرور من 8 أحرف على الأقل."); return; }
    if (result === "invalid-email") { setError("أدخل بريدًا إلكترونيًا صالحًا."); return; }
    setError("تعذر إنشاء الحساب حاليًا. حاول مرة أخرى لاحقًا.");
  }

  async function submitReset(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setSubmittingReset(true); setError(""); setResetMessage("");
    const result = await requestPasswordReset(email);
    setSubmittingReset(false);
    if (result === "sent") { setResetMessage(resetConfirmation); return; }
    if (result === "invalid-email") { setError("أدخل بريدًا إلكترونيًا صالحًا."); return; }
  ...
  const intro = mode === "signin" ? "تابع طلباتك ورصيدك من حسابك، أو أدر المتجر من لوحة CMC بحسب صلاحية حسابك." : mode === "signup" ? "أنشئ حسابك لمتابعة طلباتك ورصيدك ورسائل الدعم من مكان واحد." : "أدخل البريد الإلكتروني المرتبط بحسابك وسنرسل لك رابطًا آمنًا لإعادة التعيين.";
